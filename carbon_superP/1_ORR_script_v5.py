...
import matplotlib.pylab as pl # import matplotlib library
...
import matplotlib._layoutbox as layoutbox # import matplotlib library
import shutil  # import shutil library
from mpl_toolkits.axes_grid1.inset_locator import inset_axes # import matplotlib library
import matplotlib.ticker as tick # import matplotlib library
from matplotlib import rcParams # import matplotlib library
...
#---------------------------------------------------------------------------------------
#Text remarks
input_delimiter= ";" # delimiter in ASCII files 
input_skip_space=2 # skip first two rows in ASCII files 
#---------------------------------------------------------------------------------------
#Graphics remarks
color_list_N2 = ['black','red','blue'] # full colors list for nitrogen files
label_list_N2=['0 rpm','1000 rpm','2000 rpm'] # full list of rotatory electrode rpm values used in N2 analysis
color_list_O2 = ['black','red','blue',"green","orange","purple","yellow","gray","pink","brown"] # full colors list for oxygen files
...
fname_output_graf="ORR_graphs" #filename of ORR plots
format_plot= ".png" #format for ORR plots
#---------------------------------------------------------------------------------------
#List N2 & O2 files
...
#---------------------------------------------------------------------------------------
#rpm list used in the ORR O2 experiments
...
#---------------------------------------------------------------------------------------
#Interpolated potential points values in ORR graph 
...
#---------------------------------------------------------------------------------------
Area_electrode=0.19625  # geometric area electrode cm2
#--------------------------------------------------------------------------------------
...
def plot_EIS_Nyquist_N2(imput,color1,label1,graph_title, plot_out):
    ...
    for file in imput:
        ...
        axins.tick_params(labelleft=True, labeltop=True,labelbottom=False) #generate ticks
...
def plot_EIS_Nyquist_O2(imput,graph_title, plot_out):
    ...
    plt.savefig(plot_out, format="png",overwrite=True) #save plot
    shutil.move(plot_out,  "figures")  #move plot to figures folder
    plt.clf()
...
def plot_EIS_Bode_phase_modulus_N2(imput,color1,label1,graph_title, plot_out):
    ...
    ax1.tick_params(axis='y', labelcolor="blue") #generate ticks
    ax1.set_yscale('log')
...
def plot_EIS_Bode_phase_modulus_O2(imput,graph_title, plot_out):
    ...
    ax1.set_yscale('log')
    ax2.set_ylabel('Phase \N{DEGREE SIGN}', color="red")  # we already handled the x-label with ax1
    ax2.tick_params(axis='y', labelcolor="red") #generate ticks
    fig.tight_layout()  # otherwise the right y-label is slightly clipped
...
def subtract(imput,plot_out,data_out):
    """This function subtract Nitrogen  file to Oxigen  file data
    ...
    for file in imput[1:5]:
        ...
        plt.plot(E_RHE,(right_WE_current_corrected*1000000/Area_electrode),color=color_list_O2_2[counter_1],label=label_list_O2_2[counter_1])

        counter_1=counter_1+1 #modified
    ...
    for file in imput[5:]:
        ...
        plt.plot(E_RHE, (right_WE_current_corrected*1000000/Area_electrode),color=color_list_O2_3[counter_2],label=label_list_O2_3[counter_2]) #color=color_list_O2_3[contador_2],label=label_list_O2_3[contador_2]

        counter_2=counter_2+1 #modified
...
def Halfwave_potential_reduction_branch(imput):
    ...
    shutil.move("figure_anodic.png",  "figures") #move plot to figures folder
    plt.clf()
...
def Limiting_current_E_onset(imput):
    ...
    shutil.move("figure_limiting_current.png",  "figures") #move plot to figures folder
    plt.clf()
...
def move_data():
    ...
    for f in files:
     shutil.move(f, "data") #move file to data folder
